...
async function analyzeGreyTones(imagePath) {
  try {
    ...
    for (let i = 0; i < data.length; i += 4) { // Process RGBA quads
      ...
      // Verify it's actually grayscale (R=G=B)
      if (r !== g || g !== b) {
        console.log(`Warning: Image is not purely grayscale at pixel ${i/4}. R=${r}, G=${g}, B=${b}`);
      }
      
      greyValues.add(r); // Since it's grayscale, R=G=B
    }
...
async function runAnalysis() {
  ...
  try {
    // Analyze both images
    const image1Data = await analyzeGreyTones('./image0000001.png');
    const image2Data = await analyzeGreyTones('./image000002.png');
    
    // Process both images with the updated workflow
    await processImageWithWorkflow(image1Data.imageData, 'image000001.png');
    await processImageWithWorkflow(image2Data.imageData, 'image000002.png');
